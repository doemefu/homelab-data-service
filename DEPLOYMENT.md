...
| `/status`: `egress` with `lastErrorCode=truncated` | An hour had more than `netmon.egress.max-rows-per-window` (2 000) flows; the smallest by bytes sent were dropped (WARN `[egress] window end=… kept … of … rows`). Raise the property or look for a workload with a destination explosion |
